...
protected $description = 'Migrate the documents from a driver to another driver.';
	...
	$Bar = $this->output->createProgressBar($count);
		...
		for($i = 0; $i < $count; $i++)
		{
		$pathSourceFrom = App::make('document.driver')->driver($this->argument('from'))->pathSource($documents[$i]['idDocument'], $documents[$i]['format']);
		App::make('document.driver')->driver($this->argument('to'))->create($documents[$i]['idDocument'], $documents[$i]['format'], $pathSourceFrom);
		$Bar->advance();
		}
